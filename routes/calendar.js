   ...
   Calendar.create({
     ...
     if(user.calendars) {
      ...
      console.log(userCalendar._id);
         User.updatemany({_id: user.id},{$push: {
      ...
      //   user.calendars = [{calendarId: calendar._id}];
         User.updatemany({_id: user.id},{$addToSet: {
           calendars: {calendarId: userCalendar._id}
          }}, function(err,userCalendar){
            if(err){
              console.log(err)
    ...
    Calendar.create({
        ...
        User.findOne({_id: req.body.user.id},function(err,user){
            if(user.calendars) {
                ...
                console.log("did this cal add...from set");
                User.updatemany({_id: req.body.user.id},{$addToSet: {
                    calendars: {calendarId: userCalendar._id}
                }}, function(err,userCalendar){
                    if(err){
                        console.log(err)
                ...
                holidays.map((holiday) => {
                  ...
                  //   console.log('start', holiStart, typeof holiStart);
                    ...
                    if(calendar.events){
                        ...
                        Calendar.updatemany({ _id: calendar_id },
                            { $addToSet: {
                                events: {
                                    name: holiName,
                                    startDate: holiEnd,
            ...
            User.findOne({email: req.body.email}, function(err,contributor){
                ...
                if(newContributor._id){
                    if(calendar.people){
                        ...
                        Calendar.updatemany({_id: calendar._id},{
                            $addToSet:
                                {people:{userId:newContributor._id, permission:req.body.permission}}
                        },function(err,newPerson){
                            if(err){
                    ...
                    User.updatemany({email: req.body.email},{$push:{calendars:{calendarId: req.body.calendarId}}},function(err,updatedUser){
                        if(err){
                            console.log(err);
                        }
            ...
            for(let i=0; i<calendar.people.length; i++){
                if(calendar.people[i].userId == req.body.user.id && calendar.people[i].permission == 'edit'){
                    ...
                    User.findOne({email: req.body.email}, function(err,contributor){
                        ...
                        newContributor = contributor;
                        if(newContributor._id){
                            Calendar.updatemany({_id: calendar._id},{$push:{people:{userId:newContributor._id, permission:req.body.permission}}},function(err,newContributor){
                                if(err){
                                    console.log(err);
                                }
                            });
...
router.post('/editName', function(req,res,next){
    Calendar.findOne({_id: req.body.calendar},function(err,calendar){
        ...
        if(calendar.userId == req.body.user.id){
            Calendar.updatemany({_id: req.body.calendar},{$set: {name: req.body.name}},function(err,updatedCalendar){
                if(err){
                    console.log(err);
                }
